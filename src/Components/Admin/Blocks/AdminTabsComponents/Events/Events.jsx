    ...
    return (
        ...
        >
            ...
            <div className={classes.multidayTours_data__tour___btns}>
                <Link to={`editEvent/${tour._id}`} className={`${classes.multidayTours_data__tour___btns____item} ${classes.editBtn}`}>
...
function Events({ children, title, type, ...props }) {
    ...
    const response = () => {
        fetch(`${server}/api/getEvents?region=${title}&filter='-'`)
            .then(response => response.json())
            .then(data => {
                const sortedTours = data.events.sort((a, b) => a.order - b.order);
    ...
    const saveOrder = (updatedTours) => {
        ...
        fetch(`${server}/api/updateEventsOrder`, {
            method: 'POST',
            headers: {
    ...
    const deleteElement = (id) => {
        fetch(`${server}/api/deleteEvent/${id}`, {
            method: 'DELETE'
        })
            .then(() => {
                response();
                ...
                <DndProvider backend={HTML5Backend}>
                    <div className={classes.multidayTours}>
                        ...
                        <div className={classes.multidayTours_top}>
                            ...
                            <Link to={'addEvent'} className={classes.multidayTours_top__add}>Добавить Ивент</Link>
                        </div>
                ...
                : <>
                    {add === 'addEvent' ?
                        <>
                            ...
                            <AddEvent region={title} onTourAdded={response} />
                        </>
                        :
                        <>
                            <div className={`${classes.multidayTours_back} ${classes.mb40}`}>
                            ...
                            <EditEvent region={title} onTourAdded={response} photoMassName={'photos'}/>
                        </>
                    }
                </>
            }
