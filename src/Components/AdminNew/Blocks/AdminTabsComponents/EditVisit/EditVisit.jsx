...
function EditVisit({
  ...
  const changeMainImg = async (id, photoPath) => {
    if (confirm('Вы уверены, что хотите сделать эту картинку главной?')) {
      try {
        await fetch(
          `${server}/api/changeMainImgPlace?id=${id}&mainImgPath=${photoPath}`,
          {
            method: 'PUT',
          }
        );
  ...
  return (
    <div className={classes.addData}>
      ...
      <FormEdit
        actionUrl={`${server}/api/updateOnePlace/${idToEdit}`}
        method="put"
      ...
      >
        ...
        <input
          name="mapLink"
          type="text"
          placeholder="Ссылка из карт на место"
          value={selectedPlace.mapLink}
        />
        ...
        <div className={classes.imgBlock}>
          {loadedPhotos.map((photo, index) => (
            <div className={classes.imgBlock__item} key={index}>
              ...
              <div
                className={classes.imgBlock_close}
                onClick={() => handleRemovePhoto(index, photo)}
              >
                <img src="/delete.webp" alt="Delete" />
              </div>
              <div
                className={`${classes.imgBlock_checked} ${
                  selectedPlace.mainPhoto === photo
                    ? classes.imgBlock_checked_show
                    : null
                }`}
                onClick={() => changeMainImg(idToEdit, photo)}
              >
