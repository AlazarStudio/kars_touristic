  ...
  const selectAllUsers = () => {
    setSelectedUsers((prev) => {
      if (prev.size === agents.length) {
        ...
        return new Set(agents.map((user) => user.id));
      }
    });
  };
    ...
    <div className={classes.multidayTours}>
      <div className={classes.gidsButton}>
        ...
        <div className={classes.gidsButtonRight}>
          <button onClick={deleteSelectedUsers} disabled={selectedUsers.size === 0}>Удалить</button>
          <button onClick={() => handleEditUser(agents.find(u => selectedUsers.has(u.id)))} disabled={selectedUsers.size !== 1}>Редактировать</button>
        </div>
      </div>
