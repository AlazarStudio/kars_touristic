  ...
  useEffect(() => {
    fetch(`${server}/api/getRegions`)
      ...
      .then(data => {
        ...
        setRegions(sortedRegions.map(region => region.name));
      })
      .catch(error => console.error('Ошибка при загрузке регионов:', error));
  }, []);

  useEffect(() => {
    sections.find(section => section.title === 'Страницы').items.find(item => typeof item === 'object' && item.title === 'Регион').subItems = regions;
  }, [regions]);
      ...
      <div className={classes.container}>
        <div className={classes.containerLeft}>
          <a href="/" target="_blank" className={classes.containerLeftA}>
            <img src="/about_title_logo.webp" alt="" className={classes.containerLeftLogo} />
          </a>
                ...
                {openSections[section.title] && (
                  <div className={classes.sectionContent}>
                    {section.items.map((item) =>
                      typeof item === 'string' ? (
                        <div key={item} className={classes.sectionItem}>{item}</div>
                      ) : (
                          ...
                          {openSubSections[item.title] && (
                            <div className={classes.subSectionContent}>
                              {item.subItems.map((subItem) => (
                                <div key={subItem} className={classes.subSectionItem}>{subItem}</div>
                              ))}
                            </div>
                          )}
                        </div>
